...
public class AllCouriersPM implements CourierEventListener {
    ...
    private ObservableSet<CourierPM> allCouriersSet = FXCollections.observableSet();
    private ObservableList<CourierPM> syncAllCouriers = FXCollections.synchronizedObservableList(allCouriers);
    //private ObservableSet<CourierPM> syncAllCouriersSet = FXCollections.synchronizedObservableSet(allCouriersSet);
    ...
    public void updateAllCouriersPM(CourierPM pm){
        for(int i= 0; i < syncAllCouriers.size(); i++){
            CourierPM currentObj = syncAllCouriers.get(i);
           if(currentObj.getCourierId().equals(pm.getCourierId())){
                syncAllCouriers.remove(currentObj);
                break;
           }
    ...
    public void handleNewCourierEvent(CourierPM courierInfo) {
        syncAllCouriers.add(courierInfo);
    }
